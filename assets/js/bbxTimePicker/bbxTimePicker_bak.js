 ...
 */
function bbxDatePicker(obj, callback){
    //初始化页面当前时间
    var curTime = new Date();
    obj.val('现在出发 ' + doubleNumber(curTime.getHours()) + ':' + doubleNumber(curTime.getMinutes())).attr('data-value', formatDate(curTime, 'yyyy-MM-dd HH:mm:ss'));
    //选择出发日期
    ...
    function done(){
        ...
        setTimeout(function(){
            ...
            obj.val(timeText).attr('data-value', time);
            if(callback){
                callback();
            };
        },500);
