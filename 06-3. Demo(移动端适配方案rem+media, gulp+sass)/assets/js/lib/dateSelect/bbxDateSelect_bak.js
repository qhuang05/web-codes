...
var bbxDatePicker = {
    run: function(){
        ...
        $('li.time input').val('现在出发 ' + this.doubleNumber(curTime.getHours()) + ':' + this.doubleNumber(curTime.getMinutes())).data('value', this.formatDate(curTime, 'yyyy-MM-dd HH:mm:ss'));
        //选择出发日期
        var _this = this;
        $('li.time input').off('click').on('click', function(){
            _this._bbxDatePicker(curTime);
        });
    },

    _bbxDatePicker: function(curTime){
        var datesData = this.generateDateList(curTime);
        this.showTimePicker(datesData);
        this.showSlotsBySelectedValue(curTime);
        var _this = this;
        $('#sw-wrapper').on('touchend', function(event){
            setTimeout(function(){
                _this.showSlotsBySelectedValue(curTime);
    ...
    generateDateList: function(curTime){
        ...
        datesData.dates = {};
        for(var i=0; i<24; i++){
            datesData.hours[i] = {value: i, text: this.doubleNumber(i)};
        }

        var datesArr = [];
        datesArr[0] = {
            value: this.formatDate(curTime, 'yyyy/MM/dd HH:mm'),
            text: '现在出发'
        };
        ...
        if(!(curTime.getHours() == 23 && curTime.getMinutes() >= 50)){    //当前日期在23:50之前时, 日期列表显示今天的数据
            var item = {
                value: this.formatDate(curTime, 'yyyy/MM/dd'),
                text: this.formatDate(curTime, 'MM月dd日') + ' 今天'
            }
            datesArr.push(item);
        }
        for(var i=1; i<totalDays; i++){
            ...
            switch(i){
                ...
                case 2:
                    ...
                    break;
                default:
                    weekText = this.toWeekString(date.getDay());
                    break;
            }
            var item = {
                value: this.formatDate(date, 'yyyy/MM/dd'),
                text: this.formatDate(date, 'MM月dd日') + ' ' + weekText
            };
            datesArr.push(item);
        }
            ...
            if(selectedDate.keys[0] == 1 && selectedDate.values[0].text.indexOf('今天') != -1){   //日期选择第2列时,若第2列的日期是今天, 选择的小时和分钟数(第2、3列)不能早于当前时间
                ...
                if(parseInt(selectedDate.values[1].value) <= curHour){
                    ...
                    if(curMinute + 10 > 60){
                        ...
                        var bMinute = Math.floor(curMinute/10) * 10 + 10;
    ...
    //确定
        ...
        setTimeout(function(){
            ...
            $('li.time input').val(timeText).data('value', time);
        },500);
    },
