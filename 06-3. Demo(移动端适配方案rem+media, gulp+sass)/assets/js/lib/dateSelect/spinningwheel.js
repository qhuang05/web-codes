...
(function() {
	var dummyStyle = document.createElement('div').style,
		...
		create: function () {
			...
			div.innerHTML = '<div id="sw-header"><div id="sw-cancel">取消</' + 'div><div id="sw-done">确定</' + 'div></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div>';
			...
			for (l = 0; l < this.slotData.length; l += 1) {
				...
				for (i in this.slotData[l].values) {
					if(this.slotData[l].values.hasOwnProperty(i)) {
						out += '<li data-value="' + this.slotData[l].values[i].value + '">' + this.slotData[l].values[i].text + '<' + '/li>';
					}
				}
				ul.innerHTML = out;
		...
		destroy: function () {
			...
			(this.container || document.body).removeChild(document.getElementById('sw-mask'));      //销毁遮罩
		},
		...
		scrollTo: function (slotNum, dest, runtime) {
			...
			if(isIDevice){	//解决在某些Android手机上滚动迟缓问题
				this.slotEl[slotNum].style[transitionDuration] = runtime ? runtime : '100ms';
			}
